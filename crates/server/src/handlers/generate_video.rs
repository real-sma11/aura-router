//! Video generation handler — generates videos via Google Veo, uploads to S3.
...
use aura_router_proxy::{billing, storage, veo};
...
pub async fn generate_video_stream(
    ...
    let cost_cents = veo::cost_cents(&input.model, &input.resolution, input.duration_seconds);
    ...
    let (event_tx, mut event_rx) = tokio::sync::mpsc::channel::<veo::VideoStreamEvent>(32);
    ...
    tokio::spawn(async move {
        // Start event
        let _ = event_tx
            .send(veo::VideoStreamEvent::Start {
        ...
        .await
        {
            Ok(name) => {
                let _ = event_tx
                    .send(veo::VideoStreamEvent::Progress {
                        percent: 5,
            ...
            Err(e) => {
                let _ = event_tx
                    .send(veo::VideoStreamEvent::Error {
        ...
        let _ = event_tx
            .send(veo::VideoStreamEvent::Completed { video_url, meta })
            .await;
    });
...
/// GET /v1/generate-video/config — Available models.
pub async fn generate_video_config(
    _auth: AuthUser,
) -> Json<veo::VideoGenConfig> {
    Json(veo::get_config())
}
